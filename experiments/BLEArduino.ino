...
if(diff < -0.5){
  ...
  // if 500ms have passed, check:
   ...
   //   floatValueCharacteristicX.writeValue(current_x);
    //   floatValueCharacteristicY.writeValue(current_y);
    if (tap==1.0)  // send tap when when detected
    {
...
//  else{   // to be used if we want to send any default value when no tap is detected (overkill)
//    floatValue=-100.00;
//    floatValueCharacteristicX.writeValue(floatValue);
//     floatValueCharacteristicY.writeValue(floatValue);
//     floatValueCharacteristicZ.writeValue(floatValue);
//  }
 }
}
